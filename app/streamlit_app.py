    ...
    if os.path.exists(metrics_path):
        try:
            ...
            if "threshold" in metrics:
                ...
                config["anomaly_detection"]["score_threshold"] = float(
                    metrics["threshold"]
                )
        except Exception:
            pass
...
def predict_image(image, model, scorer, gradcam, config, device):
    ...
    # Confidence
    confidence = min(
        abs(anomaly_score - threshold) / max(threshold, 1e-6), 1.0
    )
...
def main():
    ...
    <div class="main-header">
        ...
        <p>Vision-based anomaly detection using multi-layer feature extraction + Mahalanobis scoring</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        st.markdown(
            ...
            "**Method:** Multi-Layer Features + Mahalanobis\n\n"
            "**Backbone:** ResNet-50\n\n"
            "**Dataset:** MVTec AD"
        )
    ...
    with col2:
        ...
        if uploaded_file is not None:
            # Load model
            model, scorer, gradcam, memory_bank, config, device = (
                load_model_and_scorer(config_path, category)
            )
            ...
            if model is None or scorer is None:
                ...
                with st.spinner("🔄 Analyzing image..."):
                    result = predict_image(
                        image, model, scorer, gradcam, config, device
                    )
                ...
                threshold_norm = min(
                    result["threshold"] / max(result["threshold"] * 2, 1e-6),
                    1.0,
                )
